...
$niskiestany=mysqli_query($connect,"SELECT * FROM `sppwr` WHERE `miejsce_zamocowania`='{$prze}' LIMIT 0 , 1");
while($zakresy=mysqli_fetch_array ($niskiestany)){
$zakres_min=$zakresy['zakres_min'];
}
...
$naglowek = "Wysokosc lustra wody, lokalizacja - {$nazwa}";
$niskistan= "Stan wody niski, poza zasiegiem";
...
$txt_npm_tmp = $i+$npm;
$txt_npm = "{$txt_npm_tmp}npm";

//$txt = "{$txt_h}/{$txt_npm}";
imageline($image,38,$wys_w_h-20-($i*12),42,$wys_w_h-20-($i*12),$czarny);
ImageString($image,2,0,$wys_w_h-28-($i*12),$txt_npm,$czarny);
ImageString($image,2,45,$wys_w_h-28-($i*12),$txt_h,$czarny);
}
...
ImageString($image,2,27+($i*40),$wys_w_h-12,$txt_d,$czarny);
}
...
imageline($image,40,$wys_w_h-20-(($stan_ost*12)/100),$szer_w_h,$wys_w_h-20-(($stan_ost*12)/100),$pomarancz);
imageline($image,40,$wys_w_h-20-(($stan_ala*12)/100),$szer_w_h,$wys_w_h-20-(($stan_ala*12)/100),$czerwony);
...
//if ($pomiary_h_t[0]<=$zakres_min){
	//ImageString($image,5,200,100,$niskistan,$niebieski);
//}
...
{
if(($pomiary_h_t[$i]>0)&&($pomiary_h_t[$i+1]>0)&&($pomiary_h_t[$i+2]>0)&&($pomiary_h_t[$i+3]>0))
{
$rys_h_1 = $pomiary_h_t[$i];
$rys_t_1 = $pomiary_h_t[$i+1];
$rys_t_1 = $czas_aktu-$rys_t_1;

$rys_h_2 = $pomiary_h_t[$i+2];
$rys_t_2 = $pomiary_h_t[$i+3];
$rys_t_2 = $czas_aktu-$rys_t_2;

imageline($image,920-(($rys_t_1/3600)*40),$wys_w_h-20-(($rys_h_1*12)/100),920-(($rys_t_2/3600)*40),$wys_w_h-20-(($rys_h_2*12)/100),$czarny);
}
}
